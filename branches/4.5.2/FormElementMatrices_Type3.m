%% Form global element stiffness matrix and mass matrix of inelastic
%  beam-column element type 3 along with shear deformations 
%  ISTF = 1: Zero Matrices
%  ISTF = 0: Formed Matrices
function [ElementStiffnessMatrix,...
          ElementMassMatrix] = FormElementMatrices_Type3(Element, ISTF)
...
% SEP : 6x6 element stiffness matrix in local coordinate
SEP = zeros(6);
SEP(1,1) = EAL;
SEP(1,4) = -EAL;
SEP(4,4) = EAL;
...
% ALMBDA: transformation matrix from local to global coordinate
DCOS(1,1) = X21 / EL;
DCOS(1,2) = Y21 / EL;
DCOS(1,3) = 0;
DCOS(2,1) = -DCOS(1, 2);
...
if ISTF == 0
    ...
    SEP = SE;
    MEP = ME;
    ElementStiffnessMatrix = ALMBDA' * SEP;  % Kq = bpq*Kp*apq
    ElementMassMatrix = ALMBDA' * MEP;       % Mq = bpq*Mp*apq
else
